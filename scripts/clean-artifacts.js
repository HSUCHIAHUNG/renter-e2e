...
async function cleanOldArtifacts() {
  try {
    ...
    for (const projectDir of projectDirs) {
      const fullProjectPath = path.join(ARTIFACTS_DIR, projectDir);
      ...
      console.log(`${projectDir}: 保留了 ${Math.min(validDirs.length, MAX_KEEP_COUNT)} 個測試結果`);
    }
  } catch (error) {
...
export default cleanOldArtifacts;